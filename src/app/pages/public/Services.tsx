  ...
  if (isLoading) {
    return (
      <div className="container-fluid bg-white py-5 min-vh-100 d-flex align-items-center justify-content-center">
        <RefreshCcw className="animate-spin text-primary" size={32} style={{ color: '#009CFF' }} />
      </div>
    );
  }
          ...
          <div className="container pt-3 pb-0">
            <nav aria-label="breadcrumb animated slideInDown">
              <ol className="breadcrumb mb-0">
                ...
                <li className="breadcrumb-item active text-primary" aria-current="page" style={{ color: '#009CFF' }}>{t('services')}</li>
              </ol>
            </nav>
          </div>
        </div>
        ...
        <div className="container">
          <div className="row g-5 align-items-center">
            <div className="col-lg-5 wow fadeIn" data-wow-delay="0.1s">
              <h1 className="mb-5">{servicesTitle.split(' ').slice(0, 2).join(' ')} <span className="text-uppercase text-primary bg-light px-2" style={{ color: '#009CFF' }}>{servicesTitle.split(' ').slice(2).join(' ') || t('services')}</span></h1>
              <p className="mb-4">
                {t('servicesDesc1')}
              </p>
              <p className="mb-5">
              ...
              <div className="d-flex align-items-center bg-light rounded overflow-hidden">
                <div className="btn-square flex-shrink-0 bg-primary" style={{ width: '100px', height: '100px', background: '#009CFF' }}>
              ...
              <div className="row g-0">
                {displayServices.map((service, idx) => (
                  <div key={idx} className="col-md-6 wow fadeIn" data-wow-delay={service.delay}>
                    <div className={`service-item h-100 d-flex flex-column justify-content-center p-4 ${service.dark ? 'bg-primary text-white' : 'bg-light text-dark'}`} style={{ background: service.dark ? '#009CFF' : '#f8f9fa' }}>
                      <div className="service-img position-relative mb-4">
                        ...
                        <h3 className={`p-2 position-absolute top-0 start-0 ${service.dark ? 'bg-primary text-white' : 'bg-light text-dark'}`} style={{ background: service.dark ? '#009CFF' : '#f8f9fa' }}>{service.title}</h3>
                      </div>
                      <p className={`mb-4 ${service.dark ? 'text-white' : 'text-muted'}`}>{service.description}</p>
                      <div className="mt-auto d-flex justify-content-between align-items-center">
                        <Link to="/contact" className={`text-uppercase fw-bold text-decoration-none d-flex align-items-center gap-2 ${service.dark ? 'text-white' : 'text-primary'}`} style={{ color: service.dark ? '#ffffff' : '#009CFF', fontSize: '13px' }}>
