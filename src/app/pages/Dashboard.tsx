import {
  X, Edit, Trash2, CalendarCheck, DollarSign, Eye, Clock,
  Briefcase, Users, Building2, Wallet, ClipboardList, CheckCircle2, Receipt, Headset
} from "lucide-react";
import {
...
export function Dashboard() {
  ...
  // State Declarations (ORDER MATTERS: define before use in hooks)
  const [statsData, setStatsData] = useState<DashboardStats | null>(() => {
    try {
      const cached = localStorage.getItem(`bsng_stats_${user?.id}`);
  ...
  useEffect(() => {
    if (statsData?.tasks) {
      setTodos(statsData.tasks as any[]);
    }
  }, [statsData]);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        setStatsData(data);
        localStorage.setItem(`bsng_stats_${user?.id}`, JSON.stringify(data));
      } catch (error) {
    ...
    if (isAdminLike || isAuditor || isEditor || isClient || isEmployee) {
      fetchStats();
      fetchActivities();
      loadMessages();
    } else {
  ...
  }, [isAdminLike, isAuditor, isEditor, isClient, isEmployee, refreshKey]);
  ...
  const addTodo = async () => {
    ...
    try {
      const newTask = await fetchApi<any>('/tasks', {
        method: 'POST',
        body: JSON.stringify({ title: todoInput.trim() })
      });
  ...
  const toggleTodo = async (id: string, isDone: boolean) => {
    try {
      ...
      setTodos(prev => prev.map(item => item.id === id ? { ...item, isDone: !isDone } : item));
    } catch (err) {
  ...
  const employeeStats = [
    { name: "My Assignments", value: (statsData as any)?.employeeStats?.[0]?.value || "0", icon: ClipboardList },
    { name: "Hours Worked", value: (statsData as any)?.employeeStats?.[1]?.value || "0h", icon: Clock },
    { name: "Pending Tasks", value: (statsData as any)?.employeeStats?.[2]?.value || "0", icon: CheckCircle2 },
    { name: "Current Salary", value: (statsData as any)?.employeeStats?.[3]?.value || "RWF 0", icon: Wallet },
  ];

  const clientStats = [
    { name: "My Properties", value: (statsData as any)?.clientStats?.[0]?.value || "0", icon: Building2 },
    { name: "Total Paid", value: (statsData as any)?.clientStats?.[1]?.value || "RWF 0M", icon: DollarSign },
    { name: "Payment Status", value: (statsData as any)?.clientStats?.[2]?.value || "Paid", icon: CheckCircle2 },
    { name: "Support Tickets", value: (statsData as any)?.clientStats?.[3]?.value || "0", icon: Headset },
  ];
  ...
  return (
    <div className="container-fluid py-0 mt-1 min-vh-100" style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)' }}>
                  ...
                  <div key={todo.id} className={`d-flex align-items-center py-2 ${i < todos.length - 1 ? 'border-bottom' : ''}`}>
                    ...
                    <div className="w-100 ms-3">
                      <div className="d-flex w-100 align-items-center justify-content-between">
                        <span className="small">{todo.isDone ? <del className="text-muted">{todo.title}</del> : todo.title}</span>
                        <button
                          className={`btn btn-sm ${todo.isDone ? 'text-primary' : ''}`}
      ...
      {(isAdminLike || isAuditor || isClient) && activities.length > 0 && (
        <div className="px-lg-4 mb-2">
