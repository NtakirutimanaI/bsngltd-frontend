...
export function NotificationBell() {
  ...
  const { user } = useAuth();
  const { getFilteredNotifications, unreadCount: totalUnread, markAsRead, markAllAsRead, deleteNotification } = useNotifications();

  // Scoped notifications - enforce email-only tracking for everyone
  const notifications = getFilteredNotifications(user?.email || '', '', false);
